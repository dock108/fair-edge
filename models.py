...
class Sport(Base):
    ...
    sport_id = Column(String(50), primary_key=True)
...
class Book(Base):
    ...
    # Note: No direct relationship to BetOffer
    # since we aggregate odds by bet_id
...
class Bet(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "sport", "league", "bet_type", "event_time", "sha_key", name="uq_bet_dedup"
        ),
    )
    ...
    def create_or_get_bet_id(cls, opportunity_data: Dict[str, Any]) -> str:
        ...
        parameters = {}
        bet_description = opportunity_data.get(
            "Bet Description", opportunity_data.get("bet_description", "")
        )
        ...
        if (
            "total" in bet_description.lower()
            or "over" in bet_description.lower()
            or "under" in bet_description.lower()
        ):
...
class BetOffer(Base):
    """Aggregated odds data for betting opportunities
    across all supported books"""
    ...
    draftkings_odds = Column(JSON)
    # e.g., {"american": "+110", "decimal": 2.10, "ev": 0.05}
    fanduel_odds = Column(JSON)
    # e.g., {"american": "+105", "decimal": 2.05, "ev": 0.03}
    novig_odds = Column(JSON)
    # e.g., {"american": "+115", "decimal": 2.15, "ev": 0.07}
    prophetx_odds = Column(JSON)
    # e.g., {"american": "+120", "decimal": 2.20, "ev": 0.08}
    pinnacle_odds = Column(JSON)
    # e.g., {"american": "+108", "decimal": 2.08, "ev": 0.04}
    ...
    refresh_cycle_id = Column(String(36))
    # ID linking offers from same refresh
...
class UserProfile(Base):
    ...
    # Current App Store transaction ID
    apple_original_transaction_id = Column(String(255))
    # Original transaction ID for subscription group
    apple_receipt_data = Column(Text)
    # Latest receipt data from App Store
    apple_subscription_group_id = Column(String(255))
    # Apple subscription group identifier
    apple_purchase_date = Column(DateTime(timezone=True))
    # Date of purchase/subscription start
    apple_expires_date = Column(DateTime(timezone=True))
