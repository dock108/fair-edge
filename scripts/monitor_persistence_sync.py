#!/usr/bin/env python3
"""
Fair-Edge Data Persistence Monitoring Script (Sync Version)

This script checks the status of database persistence and provides insights
into how the data collection is performing using synchronous database access.
"""
import sys
import os
...
def check_persistence_status():
    ...
    print("🔍 Fair-Edge Data Persistence Status Check (Sync)")
    print("=" * 50)
    
    try:
        with get_sync_pgbouncer_session() as session:
            # Check total records
            result = session.execute(text("SELECT COUNT(*) FROM bets"))
            total_bets = result.scalar()
            print(f"📊 Total Bets: {total_bets:,}")
            
            result = session.execute(text("SELECT COUNT(*) FROM bet_offers"))
...
def main():
    ...
    try:
        ...
        status_ok = check_persistence_status()
        
        if status_ok:
            # Test persistence service
            test_ok = test_persistence_service()
            
            print("\n" + "=" * 50)
            if test_ok:
                print("🎉 Persistence system is working correctly!")
                print("✅ Data collection is active and functional")
                print("✅ pgbouncer compatibility issue resolved with sync service")
            else:
