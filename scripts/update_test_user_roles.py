...
async def update_test_user_roles():
    ...
    if not service_key.startswith("eyJ"):
        raise ValueError(
            "Invalid SUPABASE_SERVICE_ROLE_KEY format. Expected JWT token starting with 'eyJ'"
        )
    ...
    for email, role, status in role_updates:
        try:
            result = (
                ...
                .update({"role": role, "subscription_status": status})
                .eq("email", email)
                .execute()
            )
    ...
    try:
        profiles = (
            supabase.table("profiles")
            .select("*")
            .in_("email", [u[0] for u in role_updates])
            .execute()
        )
        ...
        if profiles.data:
            ...
            for profile in profiles.data:
                logger.info(
                    f"   - {profile['email']}: {profile['role']} ({profile['subscription_status']})"
                )
        else:
