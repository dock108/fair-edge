...
class FairOddsCalculator:
    ...
    def find_best_payout_for_outcome(
        self,
        outcome_name: str,
    ...
    def get_anchor_book_odds_pair(
        ...
        if best_a:
            ...
            odds_b_from_a = self._get_opposite_odds(
                outcome_b, anchor_book_a, market_odds, market_key
            )
            if odds_b_from_a:
        ...
        if best_b:
            ...
            # Get the opposite side from the same book
            odds_a_from_b = self._get_opposite_odds(
                outcome_a, anchor_book_b, market_odds, market_key
            )
            if odds_a_from_b:
    ...
    def calculate_fair_odds(
        self, market_odds: Dict[str, List[Dict[str, Any]]], market_key: str = None
        ...
        fair_odds_result: Dict[str, Dict[str, Any]] = {
            "outcomes": {},
            "anchor_books": {},
            "raw_probabilities": {},
        }
        ...
        if len(fair_odds_result["outcomes"]) == 2:
            ...
            fair_odds_result["raw_probabilities"][missing_outcome] = {"fair": fair_prob_missing}
        ...
        for outcome_name, american_odds in outcomes.items():
            if american_odds > 0:
                formatted_parts.append(
                    f"{outcome_name} {MathUtils.format_american_odds(american_odds)}"
                )
            else:
                formatted_parts.append(
                    f"{outcome_name} {MathUtils.format_american_odds(american_odds)}"
    ...
    def get_all_current_odds_display(
        self, market_odds: Dict[str, List[Dict[str, Any]]]
    ) -> Dict[str, str]:
        """
                ...
                if len(outcomes) == 2:
                    ...
                    for outcome in outcomes:
                        ...
                        if decimal_odds and decimal_odds > 1.0:
                            ...
                            formatted_parts.append(
                                f"{name} {MathUtils.format_american_odds(american_odds)}"
                            )
                        else:
