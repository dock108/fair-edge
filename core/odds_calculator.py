...
class OddsCalculator:
    ...
    def remove_vig(self, odds_list: List[float]) -> List[float]:
        ...
        if len(odds_list) == 2:
            ...
            return [
                MathUtils.probability_to_decimal(fair_prob1),
    ...
    def find_ev_opportunities(self, market_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        market_odds: Dict[str, Dict[str, List[Dict[str, Any]]]] = {}
                    ...
                    if outcome_name in fair_odds:
                        ...
                        if ev_percentage > 1.0:
                            opportunity = {
                                ...
                                "bookmaker_name": SportsConfig.BOOKMAKERS.get(
                                    bookmaker_key, {}
                                ).get("name", bookmaker_key),
            ...
            for bookmaker_key, outcomes in bookmaker_outcomes.items():
                for outcome in outcomes:
                    if outcome["name"] in fair_odds:
                        ev = self.calculate_ev_percentage(
                            outcome["price"], fair_odds[outcome["name"]]
                        )
                        if ev > 1.0:
                    ...
                    for exchange in self.exchanges:
                        if exchange in SportsConfig.EXCHANGE_COMMISSIONS:
                            ...
                            suggestion = {
                                ...
                                "outcome": outcome_name,
                                "exchange": exchange,
                                "exchange_name": SportsConfig.BOOKMAKERS.get(exchange, {}).get(
                                    "name", exchange
                                ),
                                "fair_odds": fair_price,
    ...
    def _calculate_maker_profit_margin(
        self, fair_odds: float, suggested_odds: float, commission: float
    ) -> float:
    ...
    def _format_bet_description(
        self, market_key: str, outcome: Dict[str, Any], _event_data: Dict[str, Any]
    ) -> str:
