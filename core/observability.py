...
class ObservabilityManager:
    ...
    def record_cache_operation(self, operation: str, result: str):
        ...
        try:
            OPPORTUNITY_METRICS["cache_operations"].labels(operation=operation, result=result).inc()
        except Exception as e:
    ...
    def record_api_request(self, role: str, endpoint: str, status_code: int):
        ...
        try:
            OPPORTUNITY_METRICS["api_requests_by_role"].labels(
                role=role, endpoint=endpoint, status_code=status_code
            ).inc()
        except Exception as e:
    ...
    def record_refresh_duration(self, source: str, duration_seconds: float):
        ...
        try:
            OPPORTUNITY_METRICS["data_refresh_duration"].labels(source=source).observe(
                duration_seconds
            )
        except Exception as e:
    ...
    def capture_message(self, message: str, level: str = "info", **context):
        ...
        if not self.sentry_enabled:
            # Use the logger method directly instead of logger.log
