...
class OddsAPIClient:
    ...
    def _make_request(self, endpoint: str, params: Dict[str, Any]) -> Optional[Dict]:
        ...
        try:
            ...
            if 'x-requests-remaining' in response.headers:
                ...
                logger.info(f"API Quota - Remaining: {remaining}, Used: {used}, Last Cost: {last_cost}")
    ...
    def get_upcoming_events_24h(self, sport_key: str) -> List[Dict[str, Any]]:
        ...
        logger.debug(f"Fetching events for {sport_key} from {commence_time_from} to {commence_time_to}")
    ...
    def get_odds_for_event(self, sport_key: str, event_id: str, markets: List[str] = None) -> Optional[Dict[str, Any]]:
        ...
        if data:
            logger.debug(f"Retrieved odds for event {event_id} with {len(data.get('bookmakers', []))} bookmakers")
            return data
    ...
    def filter_two_sided_markets(self, odds_data: Dict[str, List[Dict[str, Any]]]) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        for sport_key, events in odds_data.items():
            ...
            for event in events:
                ...
                if filtered_bookmakers:
                    ...
                    logger.debug(f"Event {event.get('id', 'unknown')} passed filtering with {len(filtered_bookmakers)} bookmakers")
                else:
                    logger.debug(f"Event {event.get('id', 'unknown')} filtered out - no valid markets")
            
            filtered_data[sport_key] = filtered_events
            logger.info(f"Filtered {sport_key}: {len(filtered_events)} events (from {len(events)} original)")
        
        return filtered_data
    
    def _is_market_valid_for_analysis(self, market_key: str, bookmaker_markets: Dict[str, Dict[str, Any]]) -> bool:
        """
        ...
        if len(major_books_offering) < 2:
            logger.debug(f"Market {market_key} rejected: only {len(major_books_offering)} major books offering")
            return False
        ...
        logger.debug(f"Market {market_key} accepted: {len(major_books_offering)} major books, two-sided")
        return True
    ...
    def _is_two_sided_market(self, market_key: str, outcomes: List[Dict[str, Any]]) -> bool:
        ...
        if market_key.startswith(('player_', 'pitcher_', 'batter_')):
            ...
            for player_data in players.values():
                ...
                if (len(player_outcomes) == 2 and 
                    len(player_points) == 1 and
                    'over' in player_outcomes and 
                    'under' in player_outcomes):
                    valid_players += 1
    ...
    def get_quota_status(self) -> Dict[str, Any]:
        ...
        response = self.session.get(f"{self.base_url}/sports", params={'apiKey': self.api_key, 'all': 'false'})
    ...
    def _enrich_event_with_additional_markets(self, event: Dict[str, Any], sport_key: str) -> Dict[str, Any]:
