...
class DashboardActivityTracker:
    ...
    def track_dashboard_access(
        ...
        try:
            ...
            self.redis_client.hset(self.active_sessions_key, session_id, json.dumps(session_data))
            ...
            logger.info(
                f"📊 Dashboard access tracked: session={session_id}, user={user_id or 'anonymous'}"
            )
    ...
    def should_auto_refresh(self) -> bool:
        ...
        try:
            ...
            logger.info(
                "✅ Auto-refresh triggered: Dashboard is active and refresh interval reached"
            )
            return True
