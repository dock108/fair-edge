...
from services.redis_cache import store_analytics_data, store_ev_data
...
def refresh_odds_data(self, force_refresh=False, skip_activity_check=False):
    ...
    try:
        ...
        if force_refresh:
            logger.info(
                "🚀 Celery task started: Refreshing odds data for all sports (FORCE REFRESH)"
            )
        elif skip_activity_check:
            logger.info(
                "🚀 Celery task started: Refreshing odds data for all sports (BUSINESS HOURS SCHEDULE)"
            )
        else:
            logger.info(
                "🚀 Celery task started: Refreshing odds data for all sports (OFF-HOURS - ACTIVITY CHECK)"
            )
        ...
        try:
            ...
            sport_opportunities, sport_analytics = process_opportunities(
                raw_data, force_refresh=force_refresh
            )
                    ...
                    successful_sports.append(
                        {
                            ...
                            // len(SPORTS_SUPPORTED),  # Approximate split
                            "events": raw_data.get("total_events", 0)
                            // len(SPORTS_SUPPORTED),  # Rough estimate
                        }
                    )
        ...
        try:
            ...
            persistence_result = sync_bet_persistence.save_opportunities_batch(
                all_opportunities, source="celery_refresh_task"
            )
        ...
        logger.error(
            f"❌ Task failed after {processing_time:.2f}s (retry {self.request.retries}): {str(exc)}"
        )
...
def store_role_based_cache(opportunities: List[Dict[str, Any]], analytics: Dict[str, Any]):
    ...
    try:
        ...
        redis_client.setex("ev_opportunities:full", 3600, json.dumps(opportunities))

        logger.info(
            f"📦 Role-based caches updated: {len(free_opportunities)} free (main lines), {len(opportunities)} full (all markets)"
        )
...
def publish_realtime_update(opportunities: List[Dict[str, Any]]):
    ...
    try:
        ...
        logger.info(
            f"📡 Published real-time update with {len(opportunities)} opportunities at {update_payload['updated_at']}"
        )
