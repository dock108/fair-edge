...
class AppleIAPService:
    ...
    async def validate_receipt(cls, receipt_data: str, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if status_code != 0:
                error_msg = cls.RECEIPT_STATUS_CODES.get(status_code, f"Unknown error: {status_code}")
    ...
    @classmethod
    async def _process_receipt_response(cls, receipt_result: Dict[str, Any], user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            purchase_date = None
            if purchase_date_ms:
                purchase_date = datetime.fromtimestamp(int(purchase_date_ms) / 1000, tz=timezone.utc)
    ...
    async def get_user_subscription_status(cls, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "has_stripe_subscription": bool(user_data.get('stripe_customer_id')),
                "is_subscriber": current_role in ["basic", "premium"] and current_status == "active"
            }
    ...
    async def restore_purchases(cls, receipt_data: str, user_id: str) -> Dict[str, Any]:
        ...
        if result.get("valid"):
            ...
            logger.info(f"Successfully restored purchases for user {user_id}: {result.get('user_role')}")
        else:
    ...
    async def _handle_subscription_activation(cls, payload: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            supabase = get_supabase()
            user_result = supabase.table('profiles').select('id').eq(
                'apple_original_transaction_id', original_transaction_id
            ).execute()
    ...
    async def _handle_subscription_cancellation(cls, payload: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            result = supabase.table('profiles').update({
                ...
                'updated_at': datetime.now(tz=timezone.utc).isoformat()
            }).eq('apple_original_transaction_id', original_transaction_id).execute()
