    ...
    def _parse_event_time(self, opportunity: Dict[str, Any]) -> Optional[datetime]:
        ...
        for field in time_fields:
            ...
            try:
                ...
                logger.warning(f"Failed to parse time field '{field}' with value '{time_value}': {e}")
                continue
        
        # If no valid time found, log a warning and return None
        logger.debug(f"No valid event time found in opportunity: {opportunity.get('Event', 'unknown')}")
        return None
    
    def _generate_sha_key(self, event_name: str, event_time: Optional[datetime], sport_id: str) -> str:
