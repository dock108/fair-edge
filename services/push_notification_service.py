...
class PushNotificationService:
    ...
    async def register_device_token(
        ...
        try:
            ...
            existing_token = session.query(DeviceToken).filter(
                    ...
                    if await self._should_send_notification(device, opportunity):
                        ...
                        if result["status"] == "success":
                            ...
                            if device.notification_failures >= 10:
                                ...
                                logger.warning(f"Deactivated device {device.id} due to repeated failures")
                    else:
                        results.append({
                            "device_id": device.id,
                            "status": "skipped",
    ...
    async def send_system_notification(
        ...
        try:
            ...
            return {
                ...
                "devices_targeted": len(device_tokens),
                "notifications_sent": success_count,
                "users_reached": len(set(r["user_id"] for r in results if r["status"] == "success")),
                "results": results
            }
    ...
    async def _send_apns_notification(
        ...
        try:
            ...
            response = await client.post(
                ...
                json=payload,
                headers={
                    "apns-expiration": str(int(datetime.now(timezone.utc).timestamp()) + 3600),  # 1 hour expiry
    ...
    async def cleanup_inactive_tokens(self, days_inactive: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            inactive_tokens = session.query(DeviceToken).filter(
                or_(
