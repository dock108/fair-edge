...
def fetch_raw_odds_data(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        with _cache_lock:
            cache_data = {"data": result, "timestamp": datetime.now()}
...
def process_opportunities(
    ...
    try:
        ...
        for sport_key, events in raw_data["data"].items():
            ...
            for event in events:
                event_name = (
                    f"{event.get('away_team', 'Unknown')} @ {event.get('home_team', 'Unknown')}"
                )
...
def _analyze_single_market(
    ...
    try:
        ...
        # EV Analysis
        ev_analysis = ev_analyzer.analyze_market_opportunities(
            market_odds, fair_odds_result, market_key
        )
        ...
        if major_books_with_both_sides < 2:
            logger.debug(
                f"Market {market_key} filtered out: only {major_books_with_both_sides} major books have both sides"
            )
            return []
        ...
        for outcome_name in fair_odds_result["outcomes"]:
            ...
            if best_market:
                ...
                if is_exchange and best_market.get("exchange_data"):
                    ...
                    best_odds_str = (
                        f"{bookmaker_display} {original_american:+d} ({adjusted_american:+d})"
                    )
                else:
            ...
            if ev_display_original:
                ...
                ev_display = (
                    f"{ev_data['ev_display']} (after fees) | {ev_display_original} (before fees)"
                )
            else:
...
def _format_bet_description(market_key: str, outcome_name: str, market_odds: Dict) -> str:
    ...
    if market_key == "h2h":
        ...
        stat_type = stat_type_map.get(
            market_key, market_key.replace("pitcher_", "").replace("_", " ").title()
        )
        ...
        stat_type = stat_type_map.get(
            market_key, market_key.replace("batter_", "").replace("_", " ").title()
        )
