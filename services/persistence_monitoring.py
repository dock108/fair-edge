...
class PersistenceMonitor:
    ...
    def record_operation(
        ...
        self.performance_metrics[operation_type] = [
            m for m in self.performance_metrics[operation_type] if m["timestamp"] > cutoff_time
        ]
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        if recent_ops:
            ...
            if recent_success_rate < 95:
                health["warnings"].append(
                    f"Recent success rate below 95%: {recent_success_rate:.1f}%"
                )
            if recent_success_rate < 85:
                health["errors"].append(
                    f"Recent success rate critically low: {recent_success_rate:.1f}%"
                )
                health["status"] = "error"
        else:
            health["checks"]["recent_operations"] = {"value": 0, "status": "warning"}
        ...
        if "batch_save" in self.performance_metrics:
            ...
            if batch_metrics:
                ...
                if avg_duration > 10000:
                    health["errors"].append(
                        f"Average batch duration too high: {avg_duration:.0f}ms"
                    )
                    health["status"] = "error"
                elif avg_duration > 5000:
                    health["warnings"].append(
                        f"Average batch duration elevated: {avg_duration:.0f}ms"
                    )
