...
from core.auth import UserCtx, get_current_user
...
class ReceiptValidationRequest(BaseModel):
    ...
    class Config:
        schema_extra = {"example": {"receipt_data": "ewoJInNpZ25hdHVyZSIgPSAi..."}}
...
async def validate_receipt(
    ...
    try:
        ...
        result = await AppleIAPService.validate_receipt(validation_request.receipt_data, user.id)
        ...
        if result.get("valid"):
            ...
            if status_code == 21006:  # Receipt valid but subscription expired
                ...
                raise HTTPException(
                    status_code=400, detail=f"Receipt validation failed: {error_msg}"
                )
        ...
        raise HTTPException(status_code=500, detail="Receipt validation error")
...
async def restore_purchases(
    ...
    try:
        ...
        result = await AppleIAPService.restore_purchases(restore_request.receipt_data, user.id)
        ...
        raise HTTPException(status_code=500, detail="Purchase restoration error")
...
async def refresh_subscription_status(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to refresh subscription status")
...
async def get_product_configuration() -> Dict[str, Any]:
    ...
    try:
        if not settings.apple_iap_configured:
            raise HTTPException(status_code=503, detail="Apple In-App Purchase is not configured")
        ...
        raise HTTPException(status_code=500, detail="Failed to get product configuration")
