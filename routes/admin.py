...
async def update_user_role(
    ...
    try:
        ...
        update_result = (
            ...
            .update({"role": role_update.role, "updated_at": "now()"})
            .eq("id", user_id)
            .execute()
        )
        ...
        # Log the role change
        logger.info(
            f"Admin {admin_user.email} changed user {current_email} role "
            f"from {current_role} to {role_update.role}. "
...
async def get_database_statistics(db: AsyncSession) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "new_users": {"last_7_days": user_data[5], "last_30_days": user_data[6]},
        }
...
async def delete_user(
    ...
    try:
        ...
        await db.execute(text("DELETE FROM profiles WHERE id = :user_id"), {"user_id": user_id})
