...
from core.stripe import construct_webhook_event, create_checkout_session
...
async def create_stripe_checkout(
    request: CheckoutRequest, user: UserCtx = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        if user.role in ["basic", "premium"] and user.subscription_status == "active":
            raise HTTPException(status_code=400, detail="User is already an active subscriber")
        ...
        if request.plan == "basic":
            ...
            raise HTTPException(
                status_code=400, detail="Invalid plan. Must be 'basic' or 'premium'."
            )
        ...
        raise HTTPException(status_code=503, detail="Payment processing is temporarily unavailable")
    ...
    try:
        ...
        if current_result.data:
            ...
            if user.get("subscription_status") != "active":
                ...
                logger.debug(
                    f"Payment succeeded for subscription {subscription_id} - user already active"
                )
        else:
            logger.warning(f"No user found for subscription {subscription_id}")
    ...
    try:
        ...
        if subscription_id:
            ...
            if result.data:
                ...
                logger.warning(
                    f"Failed to update subscription {subscription_id} status to past_due"
                )
...
async def create_customer_portal_session(
    request: Request, user: UserCtx = Depends(require_subscriber)
):
    """
    ...
    try:
        ...
        if not result.data or not result.data[0].get("stripe_customer_id"):
            raise HTTPException(
                status_code=404, detail="No Stripe customer on file; contact support"
            )
