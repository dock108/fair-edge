...
# asyncio import removed - not used
import logging
import os
...
def get_cache_info():
    try:
        ...
        return {"status": "error", "message": f"Could not connect to Redis: {str(e)}"}
...
async def get_cache_status(request: Request, admin_user: UserCtx = Depends(require_role("admin"))):
    """
...
async def trigger_manual_refresh(
    ...
    try:
        ...
        task = refresh_odds_data.apply_async(
            queue=queue_name, priority={"low": 3, "normal": 6, "high": 9}[priority]
        )
...
async def get_task_status_endpoint(
    task_id: str, request: Request, admin_user: UserCtx = Depends(require_role("admin"))
):
    """
