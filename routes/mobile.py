...
from core.auth import UserCtx, get_current_user, get_user_or_none
from core.rate_limit import limiter
from core.settings import settings

# get_supabase import removed - not used
...
async def create_mobile_session(
    request: Request, session_request: MobileSessionRequest
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to create mobile session")
...
class NotificationPreferencesRequest(BaseModel):
    ...
    ev_threshold: float = Field(
        5.0, ge=1.0, le=20.0, description="Minimum EV percentage for alerts"
    )
    enabled_sports: List[str] = Field(
        default_factory=list, description="Sports to receive notifications for"
    )
...
async def update_notification_preferences(
    ...
    try:
        ...
        if not devices:
            return {"success": False, "message": "No registered devices found for user"}
        ...
        logger.info(
            f"Updated notification preferences for user {user.id} on {len(updated_devices)} devices"
        )
...
async def send_test_notification(
    request: Request, user: UserCtx = Depends(get_current_user)
) -> Dict[str, Any]:
    """
...
async def get_user_devices(
    ...
    try:
        ...
        return {"success": True, "device_count": len(devices), "devices": devices}
...
async def remove_device(
    request: Request, device_id: str, user: UserCtx = Depends(get_current_user)
) -> Dict[str, Any]:
    """
...
async def get_mobile_config(
    request: Request, user: UserCtx = Depends(get_user_or_none)
) -> Dict[str, Any]:
    """
