...
from core.auth import UserCtx, get_optional_user, verify_jwt_token
...
class ConnectionManager:
    ...
    async def send_opportunity_update(
        self, opportunity: Dict, target_users: Optional[Set[str]] = None
    ):
    ...
    def get_connection_stats(self) -> Dict:
        ...
        return {
            ...
            "anonymous_connections": len(self.active_connections)
            - sum(len(conns) for conns in self.user_connections.values()),
            "connections_by_user": {
                user_id: len(conns) for user_id, conns in self.user_connections.items()
            },
        }
                ...
                try:
                    ...
                    if await should_send_to_user(data, user, subscription_data):
                        ...
                        if (
                            ...
                            and data.get("data", {}).get("ev_pct", 0) >= 10.0
                        ):
                            try:
                                await push_notification_service.send_opportunity_alert(
                                    user_id=user_id, opportunity=data["data"]
                                )
                            except Exception as e:
...
async def broadcast_opportunity_update(
    opportunity: Dict,
    user: UserCtx = Depends(get_optional_user),
...
@router.get("/stats")
async def get_websocket_stats(user: UserCtx = Depends(get_optional_user)):
...
async def realtime_health_check():
    ...
    try:
        ...
        return {
            "status": "unhealthy",
            "error": str(e),
            "services": {"redis_connected": False, "websocket_service": "error"},
            "connection_stats": manager.get_connection_stats(),
        }
