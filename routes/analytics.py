...
async def get_advanced_analytics(
    ...
    try:
        ...
        if timeframe not in valid_timeframes:
            raise HTTPException(
                status_code=400,
                detail=(f"Invalid timeframe. Must be one of: " f"{', '.join(valid_timeframes)}"),
            )

        # Get analytics data from cache
        # Not used but kept for compatibility
        _ = get_analytics_data(timeframe)
        ev_data = get_ev_data()
        ...
        if include_sportsbook_analysis:
            ...
            for book, stats in sportsbook_stats.items():
                ...
                stats["positive_ev_rate"] = stats["positive_ev_count"] / max(
                    stats["opportunity_count"], 1
                )
        ...
        analytics_result["risk_analysis"] = {
            "high_ev_opportunities": len(
                [opp for opp in ev_data if opp.get("ev_percentage", 0) > 5]
            ),
            "low_risk_opportunities": len(
                [opp for opp in ev_data if 0 < opp.get("ev_percentage", 0) <= 2]
            ),
            "medium_risk_opportunities": len(
                [opp for opp in ev_data if 2 < opp.get("ev_percentage", 0) <= 5]
            ),
        ...
        analytics_result["performance_insights"] = {
            "best_value_sport": (
                max(sport_breakdown.items(), key=lambda x: x[1]["avg_ev"])[0]
                if sport_breakdown
                else "N/A"
            ),
            "most_opportunities_sport": (
                max(sport_breakdown.items(), key=lambda x: x[1]["count"])[0]
                if sport_breakdown
                else "N/A"
            ),
...
async def get_analytics_summary(
    request: Request, subscriber_user: UserCtx = Depends(require_role("subscriber"))
):
    """
