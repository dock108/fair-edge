...
class SubscriberUser(HttpUser):
    ...
    def view_premium_opportunities(self):
        ...
        with self.client.get(
            "/premium/opportunities", params=params, catch_response=True
        ) as response:
    ...
    def view_analytics(self):
        ...
        # Note: This endpoint requires authentication in real usage
        with self.client.get(
            "/api/analytics/advanced", params=params, catch_response=True
        ) as response:
            if response.status_code in [401, 403]:
                response.success()  # Expected for unauthenticated load test
...
class AdminUser(HttpUser):
    ...
    def trigger_refresh(self):
        ...
        with self.client.post(
            "/api/refresh", json={"priority": "normal"}, catch_response=True
        ) as response:
            if response.status_code in [401, 403]:
                response.success()  # Expected for unauthenticated load test
