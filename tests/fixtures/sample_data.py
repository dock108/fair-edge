...
class BettingOpportunityFactory(factory.Factory):
    ...
    event = factory.LazyFunction(lambda: f"{Faker('company').generate()} vs {Faker('company').generate()}")
    bet_desc = factory.Faker('sentence', nb_words=4)
    bet_type = factory.Faker('random_element', elements=['moneyline', 'spread', 'total', 'player_props'])
    sport = factory.Faker('random_element', elements=['NFL', 'NBA', 'MLB', 'NHL', 'Soccer'])
    ev_pct = factory.Faker('pyfloat', left_digits=2, right_digits=1, positive=True, min_value=1.0, max_value=25.0)
    ...
    best_source = factory.Faker('random_element', elements=['DraftKings', 'FanDuel', 'BetMGM', 'Caesars'])
    game_time = factory.Faker('future_datetime', end_date='+30d')
    classification = factory.LazyAttribute(lambda obj: 
        'great' if obj.ev_pct >= 15 else 
        'good' if obj.ev_pct >= 10 else 
        'fair' if obj.ev_pct >= 5 else 'poor'
    )
...
class UserFactory(factory.Factory):
    ...
    subscription_status = factory.LazyAttribute(lambda obj: 
        'active' if obj.role in ['basic', 'premium'] else 'inactive'
    )
...
class AppleReceiptFactory(factory.Factory):
    ...
    product_id = factory.Faker('random_element', elements=[
